...
export const directoryMemberInsertSchema = z.object({
  ...
  business_categories: z.array(z.string().trim().min(2).max(80)).min(1).max(5),
...
export const directoryCorrectionSchema = z.object({
  ...
  keywords_tags: z.string().trim().min(4).max(500),
  products_services: z.string().trim().min(10).max(5_000),
  specialization: correctionOptionalText,
  years_experience: z.union([z.enum(YEARS_EXPERIENCE_OPTIONS), z.literal("")]),
  price_ranges: z.array(z.enum(PRICE_RANGE_OPTIONS)).max(3),
  business_address: correctionOptionalText,
  service_area: z.array(z.enum(SERVICE_AREA_OPTIONS)).max(5),
  google_maps_link: correctionOptionalUrl,
  ...
  usp: correctionOptionalText,
  certifications: correctionOptionalText,
  awards: correctionOptionalText,
  looking_for: z.array(z.enum(LOOKING_FOR_OPTIONS)).max(4),
  preferred_categories_connect: z.array(z.string().trim().min(2).max(80)).max(50),
  target_customers: correctionOptionalText,
  referred_by: correctionOptionalText,
